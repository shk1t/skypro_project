...
# decorators.py
## модуль содержит в себе одну функцию:
### 1. log (Декоратор, логирующий успешное выполнение функции или возникшие ошибки. Логи сохраняются в файл (если указан `filename`) или выводятся в консоль.)
# аргументы функции
- `filename` (опционально):
1. тип: str | None
2. путь к файлу, в который будет записываться лог.
3. если не указан — сообщения выводятся в консоль.
**возвращает:**
декоратор log(...) возвращает обёртку, которая:
- вызывает оригинальную функцию;
- в случае успеха — логирует "имя_функции ok";
- в случае ошибки — логирует тип исключения и входные параметры;
- повторно выбрасывает исключение, чтобы его можно было обработать.

# external_api.py
## модуль содержит в себе одну функцию:
### 1. convert_amount_to_rub (Фукнция, которая принимает на вход транзакцию и возвращает сумму транзакции в рублях)
# аргументы функции
- `transaction` - транзацкия
**возвращает:**
float сумму транзакции в рублях

# utils.py
## модуль содержит в себе три функции:
### 1. load_transactions_json
### 2. load_transactions_csv
### 3. load_transactions_excel
# аргументы функций
во всех функциях принимается `filename`, путь до файла
**возвращает:**
список словарей содержаших транзакции
